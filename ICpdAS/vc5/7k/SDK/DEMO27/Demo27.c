...
void TEST_CMD(HWND hwnd, int x, int dx, int y, int dy)
{
   ...
   wRet=AnalogIn8(w7000, f7000, szSend, szReceive);

   sprintf(cShow,"2. wRet=%d, S=%s, R=%s",wRet,szSend,szReceive);
   TextOut(hdc,x*dx,(y+iLine)*dy,cShow,strlen(cShow)); iLine++;

   sprintf(cShow,"3. %f %f %f %f %f %f %f %f",f7000[0],f7000[1],f7000[2],f7000[3],f7000[4],f7000[5],f7000[6],f7000[7]);
   TextOut(hdc,x*dx,(y+iLine)*dy,cShow,strlen(cShow)); iLine++;

   sprintf(cShow,"4. Wait a moment, repeat 1000 times ");
   TextOut(hdc,x*dx,(y+iLine)*dy,cShow,strlen(cShow)); iLine++;

   w7000[6]=0;	      // string no debug
   err=0;
   ...
   fSeconds=(double)(dwEndTick-dwStartTick)/1000.0;

   sprintf(cShow,"5. err=%d, fSeconds=%f --> performance=%f",err,fSeconds,8000.0/fSeconds);
   TextOut(hdc,x*dx,(y+iLine)*dy,cShow,strlen(cShow)); iLine++;
