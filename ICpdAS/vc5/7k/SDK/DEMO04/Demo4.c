...
wRet = Send_Cmd(cPort,cBuf,wCheckSum); /* time-out = 1000 ms = 1 sec */
...
if (wRet!=NoError)
   ...
   wRet = Receive_Cmd(cPort, cResult, 1000,wCheckSum, &wT);
...
wRet = Send_Binary(cPort,cBuf,wSendLen);
...
if (wRet!=NoError)
   ...
   cResult[0]=0;
   //wRet=Read_Com_Status(cPort,cResult,&wT);
   wRet = Receive_Binary( cPort, cResult, 1000, wReceiveLen, &wT );
