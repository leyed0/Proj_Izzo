...
void TEST_CMD(HWND hwnd, int x, int dx, int y, int dy)
{
   ...
   w7000[4]=wTimeOut; // TimeOut , 0.1 second
   w7000[5]=0;	      // channel no
   w7000[6]=1;	      // string debug

   wRet=AnalogIn(w7000, f7000, szSend, szReceive);

   sprintf(cShow,"2. wRet=%d, S=%s, R=%s, f=%f",wRet,szSend,szReceive,f7000[0]);
   TextOut(hdc,x*dx,(y+iLine)*dy,cShow,strlen(cShow)); iLine++;

   sprintf(cShow,"3. Wait a moment, repeat 1000 times ");
   TextOut(hdc,x*dx,(y+iLine)*dy,cShow,strlen(cShow)); iLine++;
   ...
   sprintf(cShow,"4. err=%d, fSeconds=%f --> performance=%f",err,fSeconds,1000.0/fSeconds);
   TextOut(hdc,x*dx,(y+iLine)*dy,cShow,strlen(cShow)); iLine++;
