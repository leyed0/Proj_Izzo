...
void TEST_CMD(HWND hwnd, int x, int dx, int y, int dy)
{
  ...
  w7000[5]=0;	     // command read back
  w7000[6]=1;	     // string debug

  wRet=AnalogOutReadBack(w7000, f7000, szSend, szReceive);

  sprintf(cShow,"2. wRet=%d, S=%s, R=%s, ReadBack=%f",wRet,szSend,szReceive,f7000[0]);
  TextOut(hdc,x*dx,(y+iLine)*dy,cShow,strlen(cShow)); iLine++;

  w7000[5]=1;	     // output of current path read back
  wRet=AnalogOutReadBack(w7000, f7000, szSend, szReceive);

  sprintf(cShow,"3. wRet=%d, S=%s, R=%s, ReadBack=%f",wRet,szSend,szReceive,f7000[0]);
  TextOut(hdc,x*dx,(y+iLine)*dy,cShow,strlen(cShow)); iLine++;
