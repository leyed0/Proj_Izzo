...
if (wRet!=NoError)
   {
   sprintf(cShow,"4. Open Error, return code=%d",wRet);
   TextOut(hdc,x*dx,(y+iLine)*dy,cShow,strlen(cShow)); iLine++;
   goto ret_label;
   }

w7000[0]=cPort;    // port
w7000[1]=0x01;	   // Address
w7000[2]=0x7017;   // ID
w7000[3]=0;	   // CheckSum disable
w7000[4]=wTimeOut; // TimeOut constant
w7000[5]=0;	   // channel number
w7000[6]=1;	   // string debug

wRet=AnalogIn(w7000, f7000, szSend, szReceive);

sprintf(cShow,"5. wRet=%d, S=%s, R=%s, A/I=%f",wRet,szSend,szReceive,f7000[0]);
TextOut(hdc,x*dx,(y+iLine)*dy,cShow,strlen(cShow)); iLine++;

wRet=Close_Com(cPort);
if (wRet!=NoError)
   {
   sprintf(cShow,"6. Close Error, return code=%d",wRet);
   TextOut(hdc,x*dx,(y+iLine)*dy,cShow,strlen(cShow)); iLine++;
   }
