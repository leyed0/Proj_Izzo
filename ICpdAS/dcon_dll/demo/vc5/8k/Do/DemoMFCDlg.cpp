...
BOOL CDemoMFCDlg::OnInitDialog()
{
	...
	SetDlgItemText( IDC_Do,      TEXT("55") );  // Digital Output Value
	SetDlgItemText( IDC_Rb,      TEXT("55") );  // Read Back Value
...
void CDemoMFCDlg::OnTimer(UINT nIDEvent) 
{
	// TODO: Add your message handler code here and/or call default
     ...
     gdwBuf[5] = GetDlgItemHex( IDC_Do ); // 16-bit digital data to output
     ...
     wRtn = DigitalOut_8K(gdwBuf, gfBuf, gszSend, gszReceive);
     if (wRtn != 0)
     {
         OnActive( );
		 char szErr[80];
		 sprintf( szErr, "DigitalOut_8() Error!! Error Code: %d", wRtn );
         AfxMessageBox( szErr );
		 return;
     }
     ...
     if (wRtn != 0)
     {
          ...
          SetDlgItemHex( IDC_Rb, (WORD) gdwBuf[5] ) ;
