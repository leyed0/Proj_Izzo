...
void CDemoMFCDlg::OnTimer(UINT nIDEvent) 
{
	// TODO: Add your message handler code here and/or call default
     ...
     gdwBuf[5] = GetDlgItemHex( IDC_Do ) & 1; // digital data to output
     ...
     SetDlgItemHex( IDC_Do, (WORD)gdwBuf[5] );

     // Channel Number (Bit)
     gdwBuf[8] = GetDlgItemInt( IDC_Channel ) & 0xF;
     switch ( ((CComboBox*)GetDlgItem(IDC_ChannelMode))->GetCurSel() )
     {
     case 1:  // Increment
        gdwBuf[8] = gdwBuf[8] + 1;
        if (gdwBuf[8] >= (DWORD)wTtlChannel)
            gdwBuf[8] = 0;
        break;
     case 2:  // Decrement
        if (gdwBuf[8] == 0)
            gdwBuf[8] = wTtlChannel;
        gdwBuf[8] = gdwBuf[8] - 1;
        break;
     }
     SetDlgItemInt( IDC_Channel, gdwBuf[8], FALSE );

	 // Calling the digital output function for 8K series 
     wRtn = DigitalBitOut_8K(gdwBuf, gfBuf, gszSend, gszReceive);
     if (wRtn != 0)
     {
         OnActive( );
		 char szErr[80];
		 sprintf( szErr, "DigitalBitOut_8K() Error!! Error Code: %d", wRtn );
         AfxMessageBox( szErr );
		 return;
     }
     ...
     wRtn = DigitalOutReadBack_8K(gdwBuf, gfBuf, gszSend, gszReceive);
     if (wRtn != 0)
     {
          SetDlgItemText( IDC_Rb, TEXT("Error") );
     }
     else
          SetDlgItemHex( IDC_Rb, (WORD) (gdwBuf[5] >> gdwBuf[8]) & 1);
...
void CDemoMFCDlg::OnActive() 
{
    ...
    case 0x8060:
        ...
        wMaxValue   = 0x3F;
        break;
    case 0x8054: case 0x8055: case 0x8064:
    case 0x8065: case 0x8066: case 0x8068:
        wTtlChannel = 8;
        wMaxValue   = 0xFF;
        break;
    case 0x8056: case 0x8057:
        wTtlChannel = 16;
        wMaxValue   = 0xFFFF;
        break;
    default:
